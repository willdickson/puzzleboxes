...
def convert_to_h5(bag_filename):
    ...
    for i, protocol in enumerate(param['regions']['protocols']):
        if protocol['fly']:
            ...
            data_keys = ['elapsed_t', 'led_enabled', 'object_found','fly_x', 'fly_y', 'ball_x', 'ball_y', 'classifier','led'] 
            data_dict = {}
            for k in data_keys:
                data_dict[k] = []
    ...
    for msg_count, msg_item in enumerate(bag.read_messages(topics=['/puzzleboxes_data'])):
        ...
        for i, region_data in enumerate(msg.region_data_list):
            if i in index_to_data:
                ...
                index_to_data[i]['fly_x'].append(region_data.fly_x)
                index_to_data[i]['fly_y'].append(region_data.fly_y)
                index_to_data[i]['ball_x'].append(region_data.ball_x)
                index_to_data[i]['ball_y'].append(region_data.ball_y)
